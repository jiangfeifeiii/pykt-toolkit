...
class AttnBranch(nn.Module):
    ...
    def forward(self, q_embed_data, qa_embed_data, ta_embed_data, pid_embed_data=None):
        """
        know_emb: (B, T, 768)
        """
        ta_embed_data = self.tech_proj(ta_embed_data)
        qa_embed_data = self.fusion(torch.cat([qa_embed_data, ta_embed_data], dim=-1))
...
class Mykt(nn.Module):
    def __init__(self,
                 n_question,#concept nums
                 n_pid, #总题目数
                 d_model,
                 n_blocks,
                 dropout,
        ...
        self.predictor = GatedFusionPredictor(d_model, self.dropout)
    ...
    def forward(self, q, r, s=None, pid_data=None, qtest=False):
        ...
        #[n_questions+1, d_model]
        q_embed_diff_data = self.q_embed_diff(q)
        q_embed_data = q_embed_data + pid_embed_data * q_embed_diff_data

        #[2*n_questions+1, d_model]
        qa_embed_diff_data = self.qa_embed_diff(r)  # f_(ct,rt) or #h_rt (qt, rt)差异向量
        if self.separate_qa:
        ...
        # ====== Branch 1 ======
        h1 = self.attn(q_embed_data, qa_embed_data, ta_embed_data)  # (B, T, d)
        # ====== Branch 2 ======
        ...
        rasch_reg = 0
        c_reg_loss = 0
        total_loss = c_reg_loss + self.cl_weight*loss_cl
